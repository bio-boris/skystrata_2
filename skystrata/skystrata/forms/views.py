...
from api.celery_home.run_instance import *
...
class Add(APIView):
    ...
    def __init__(self, model, serializer, instance_name):
        self.model = model
        self.serializer = serializer
        self.instance_name = instance_name

    def get(self, request):
        return Response({'serializer': self.serializer, 'instance_name': self.instance_name})
...
class Edit(APIView):
    ...
    def get(self, request, pk):
        ...
        return Response(
            {'serializer': serializer, 'instance': instance, 'instance_name': self.instance_name})
...
class LaunchInstance(Add):
    ...
    def post(self, request):
        ...
        response_context = {'serializer': new_record, 'instance': new_record.instance, 'msg': msg,
                            'instance_name': self.instance_name}

        start_instance(new_record.instance, self.request)
...
class ListInstances(APIView):
    ...
    def get(self, request, format=None):
        templates = {template.pk: template.label for template in
                     Template.objects.filter(customer=request.user)}
        instances = {instances.pk: instances.label for instances in
                     Instance.objects.filter(customer=request.user)}
